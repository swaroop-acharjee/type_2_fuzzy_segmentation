...
type_2_u = full((N,Z,Y,X),0).tolist()
...
for _ in range(N_IT):
	...
	# Calculation of the Type-2 Fuzzy set
	for i in range(N):
		for j in range(Z):
			for k in range(Y):
				for l in range(X):
					...
					S = ((-2)*(variance(list_local_u)+0.0001))

					primary_fuzzy_set = []
					for x in list_local_u:
						temp 		= (x-local_u[i][j][k][l])**2
		...
		for j in range(Z):
			for k in range(Y):
				for l in range(X):
					...
					for r in range(N):
						su += type_2_u[r][j][k][l]
					local_u[i][j][k][l] = type_2_u[i][j][k][l]/su
...
print(img_vol[0][0][0])
print(classified_img[0][0][0])
print(final_u[0][0][0][0],final_u[1][0][0][0],final_u[2][0][0][0],final_u[3][0][0][0])
print(final_u[0][0][0][0]+final_u[1][0][0][0]+final_u[2][0][0][0]+final_u[3][0][0][0])
